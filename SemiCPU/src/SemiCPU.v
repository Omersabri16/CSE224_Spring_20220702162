...
module InstructionMemory(
    ...
    always @(*) begin
        case (address)
            32'h00000000: instruction = 32'h00a00513; // ADDI x10, x0, 10
            32'h00000004: instruction = 32'h00f00793; // ADDI x15, x0, 15
            32'h00000008: instruction = 32'h00f505b3; // ADD  x11, x10, x15
            32'h0000000c: instruction = 32'hffb58613; // ADDI x12, x11, -5
            32'h00000010: instruction = 32'h00200293; // ADDI x5,  x0, 2
            32'h00000014: instruction = 32'h005596b3; // SLL  x13, x11, x5
...
module InstructionDecoder(
    ...
    always @(*) begin
        ...
        case (opcode)
            7'b0010011: ALUop = 3'b110;
            7'b0110011: begin
                case (funct3)
                    3'b000: ALUop = 3'b010; // ADD
            ...
            default: ALUop = 3'b000;
        endcase
        // $display("DECODE: op=%b f3=%b rs1=%2d rs2=%2d rd=%2d imm=%h ALUop=%b", opcode, funct3, rs1, rs2, rd, imm, ALUop);
    end
endmodule
...
module SemiCPU(
    ...
    ProgramCounter pc(
        .clk(clk), .reset(reset), .control(pulse), .pc(pc_out)
    );
    ...
    RegFile reg_file(
        ...
        .write_reg(rd), .write_data(alu_result),
        .reg_write(pulse && (instruction != 32'b0) && (rd != 0)),
        .read_data1(reg_data1), .read_data2(reg_data2)
    );

    ALU alu(
        .A(reg_data1),
        .B((ALUop == 3'b110 || ALUop == 3'b111) ? imm : reg_data2),
        .ALUop(ALUop), .Result(alu_result)
    );

    SevenSegmentDisplay display(
        .clk(clk), .data(alu_result), .seg(seg), .an(an)
    );
endmodule
